...
class HttpRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def list_directory(self, path):
        ...
        if path in ('/', '\\'):
            # 根目录下显示磁盘驱动器
            response_content = '<!DOCTYPE html><html><head><meta charset="utf-8"><title>磁盘选择</title>'
            ...
            response_content += '</style></head><body><h2>请选择磁盘驱动器</h2><ul>'
            for drive in string.ascii_uppercase:
                if os.path.exists(f'{drive}:\\'):
                    response_content += f'<li><a href="/{drive}:/">🖴 {drive}:/</a></li>'
            ...
            response_content = '<!DOCTYPE html><html><head><meta charset="utf-8"><title>目录列表</title>'
            ...
            response_content += f'<h2>当前路径: {path}</h2>'
            response_content += '<button onclick="document.getElementById(\'fileInput\').click()">📤上传文件</button>'
            response_content += '<input id="fileInput" type="file" multiple style="display:none" onchange="uploadFile()"/>'
            response_content += '<button onclick="document.getElementById(\'folderInput\').click()">📤上传文件夹</button>'
            response_content += '<input id="folderInput" type="file" webkitdirectory style="display:none" onchange="uploadFolder()"/>'
            response_content += '<button onclick="promptNewFolder()">🆕新建文件夹</button>'
    ...
    def handle_partial_content(self, range_header):
        ...
        try:
            ...
            start_byte = int(range_value.split('-')[0])  # 分割范围值，获取开始字节的位置并转换为整数
            ...
            # 计算结束字节的位置
            end_byte = file_size - 1  # 设置结束字节为文件的最后一个字节

            # 以二进制模式打开请求的文件
            with open(path, 'rb') as f:
                # 将文件指针移动到请求的起始位置
                f.seek(start_byte)  # 定位到开始字节
                ...
                block_size = 131072  # 设置每次读取和发送的块大小为131072字节（128KB）

                # 循环读取和发送文件的指定范围内容，直到所有请求的内容被发送
                while remaining_bytes > 0:  # 循环，直到剩余的字节数为0
                    read_bytes = min(block_size, remaining_bytes)  # 确定每次从文件中读取的字节数，不超过预设的块大小
                    data = f.read(read_bytes)  # 从文件中读取数据
                    if not data:  # 如果没有读取到数据，表示文件已经读取完毕
                        break
                    try:
                        self.wfile.write(data)  # 尝试将读取到的数据发送给客户端
                        remaining_bytes -= read_bytes  # 更新剩余需要发送的字节数
    ...
    def do_POST(self):
        ...
        if 'delete' in form:  # 检查表单中是否有删除请求
            ...
            try:
                ...
                upload_details.append(f'文件 "{os.path.basename(file_path)}" 已被删除。')  # 添加操作详情
                self.send_response(200)  # 发送HTTP响应
            except OSError as e:
